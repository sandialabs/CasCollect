...
if bbduk == None:
    print('Missing BBTools, run check.py')
    sys.exit()
if seqtk == None:
    print('Missing Seqtk, run check.py')
    sys.exit()
if FGS == None:
    print('Missing FragGeneScanPlus, run check.py')
    sys.exit()
if hmmsearch == None:
    print('Missing HMMER, run check.py')
    sys.exit()
if vsearch == None:
    print('Missing VSEARCH, run check.py')
    sys.exit()
if spades == None:
    print('Missing SPAdes, run check.py')
    sys.exit()
if crisprcasfinder == None:
    print('Missing CRISPRCasFinder, run check.py')
    sys.exit()
...
parser.add_argument('-query_id', metavar='number', type=str, default=str(1.00), help='global clustering threshold for '
                                                                                     'vsearch')
...
ccfsum1 = program_path + '/bin/ccfSum1.pl'
...
if str(fwd) == 'None' and str(rev) == 'None' and str(single) == 'None':
    ...
    if meta:
        print('metaSPAdes does not support unpaired reads, please remove --meta flag')
        sys.exit()
else:
...
if read_input == 'paired' and (trim or clean):
    ...
    bbduk = open('bbduk.txt', 'w')
    subprocess.call(['bbduk.sh', 'in1=' + fwd_file,  'in2=' + rev_file, 'out1=fastq/fwd_file.fastq',
                     'out2=fastq/rev_file.fastq', 'outs=fastq/single.fastq', ref_input, 'ktrim=r', 'minlen=25',
                     'minlenfraction=0.333', 'mink=11', 'overwrite=true', 'k=23', 'threads=' + cpu], stdout=bbduk)
    bbmerge = open('bbmerge.txt', 'w')
    subprocess.call(['bbmerge.sh', 'in1=fastq/fwd_file.fastq', 'in2=fastq/rev_file.fastq',
                     ...
                     'outu2=fastq/rev_file_unmerged.fastq', 'threads=' + cpu], stdout=bbmerge)
    ...
    if trim:
        ...
        print('cleaning single-end reads')
        ref_input = 'ref=' + adapters + ',' + ref
    FNULL = open(os.devnull, 'w')
    subprocess.call(['bbduk.sh', 'in=' + single_file, 'out=fastq/single_file.fastq', ref_input,
                     'ktrim=r', 'minlen=25', 'minlenfraction=0.333', 'mink=11', 'overwrite=true', 'k=23',
                     'threads=' + cpu], stdout=FNULL)
else:
...
if nucl:
    ...
    subprocess.call(['vsearch', '--usearch_global', query, '--db', 'fasta/reads.fasta', '--minseqlength', '1', '--id',
                     query_id, '--strand', 'both', '--maxaccepts', '100', '--uc_allhits', '--userfields', 'target',
...
if noassembly or noannotate:
    ...
    if noprot:
        subprocess.call(['perl', ccfsum1, 'assembly/scaffolds.fasta', 'search', 'spades', cpu])
    else:
